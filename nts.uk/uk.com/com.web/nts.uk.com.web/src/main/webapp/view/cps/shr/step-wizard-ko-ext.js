...
(function (nts) {
    ...
    (function (custombinding) {
        ...
        var StepWizardControl = /** @class */ (function () {
            function StepWizardControl() {
                this.init = function (element, valueAccessor, allBindingsAccessor, viewModel, bindingContext) {
                    var $element = $(element), $step = valueAccessor(), $allBinding = allBindingsAccessor(), $header = $('<div>', { 'class': 'header' }).appendTo($element), $icon = $('<div>', { 'class': 'img' }).appendTo($header), $title = $('<h5>', {}).appendTo($header), $steps = $('<ul>', { 'class': 'step-list list-group' }).appendTo($element), $style = $('<style>', {
                        'type': 'text/css',
                        'rel': 'stylesheet',
                    ...
                    ko.computed({
                        read: function () {
                            var step = ko.toJS($allBinding.selected), steps = ko.toJS($step);
                            $steps.empty();
                            _.each(steps, function (s, i) {
            ...
            return StepWizardControl;
        }());
        custombinding.StepWizardControl = StepWizardControl;
...
//# sourceMappingURL=step-wizard-ko-ext.js.map